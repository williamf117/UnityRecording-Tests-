                    ...
                    for (int i = 0; i < all.Length; i++)
                    {
                        ...
                        GUILayout.BeginHorizontal(EditorStyles.helpBox, GUILayout.Height(10));
                        {
                            ...
                            if(Replay.IsComponentObserved(all[i]) == false)
                            {
                                ...
                                GUILayout.Label(new GUIContent("[Not Recorded]", "This component is not recorded because it is marked with the 'ReplayIgnore' attribute or is managed by a lower level ReplayObject"), labelStyle);
